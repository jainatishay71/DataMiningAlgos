...
void getcount(map<vector<int> ,int> &m,string tf)
{
	ifstream fin(tf.c_str());
	int x=0,n;
	while(fin>>n)
	{
		if(x==0)
...
void putinfile(map<vector<int> ,int> &m,int lc,int rel_sup,map<vector<int> ,int> &partans)
{
...
void modifytemp(map<vector<int> ,int> &temp,string tf)
{
...
int create_partitions()
{
	...
	if(ttrans%nop==0)
		...
		tlast=ttrans%nop;
	int mfn=1,q=0,c=0;
	string mf="mf",ext=".txt";
	string tf=mf+itoa(mfn)+ext;
	ofstream mfout;
	while(fin>>n)
	{
		if(x==0)
...
int doapriori(string tf,int lc,int rel_sup,map<vector<int> ,int> &partans)
{
	map<vector<int> ,int> m;
	map<vector<int> ,int>::iterator mit,it,tit;
	getcount(m,tf);
	putinfile(m,lc,rel_sup,partans);
	int lastfileno=0;
	while(1)
	{
		lastfileno++;
...
int main()
{
	srand(time(NULL));
	generate_transdb();
	int nop=create_partitions();
	map<vector<int> ,int> partans;
	map<vector<int> ,int>::iterator mit;
	for(int i=1;i<=nop;i++)
	{
		map<vector<int> ,int> m;
		string tf="mf"+itoa(i)+".txt";
		int rel_sup;
		if(i!=nop)
		{
			rel_sup=ceil((double)(min_sup*teach)/(double)ttrans);
			int num=doapriori(tf,i,rel_sup,partans);
		}
		else
		{
			rel_sup=ceil((double)(min_sup*tlast)/(double)ttrans);
			int num=doapriori(tf,i,rel_sup,partans);
		}	
	}
	ofstream fout("allfreq.txt");
	for(mit=partans.begin();mit!=partans.end();mit++)
	{
		if(mit->second>=min_sup)
		{
			...
			{
				fout<<mit->first[i]<<" ";
				if(i==mit->first.size()-1)
				{
					fout<<"\t-->\t";
				}
			}
			fout<<mit->second<<endl;
		}
	}
	fout.close();
}
